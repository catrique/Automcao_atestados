...
class BethaService:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.inicializado = False
            self.erro_inicializacao = (
                f"❌ Erro ao carregar configurações do Betha: {str(e)}"
            )
    ...
    def buscar_matricula(self, termo) -> OperationResult:
        ...
        params = {"filter": filtro, "filtroSituacao": "TODOS", "limit": 20}

        try:
            res = sessao_limpa.get(
                url,
            ...
            if e.response.status_code in [401, 403]:
                return OperationResult.fail(
                    "🔑 Token da Betha expirado ou inválido. Por favor, atualize o token."
                )
    ...
    def processar_lote_planilha(self, dados_planilha) -> OperationResult:
        ...
        return OperationResult.ok(
            f"{len(linhas_filtradas)} registros prontos.", data=linhas_filtradas
        )
    ...
    def gerar_payloads_lote(self, linhas_filtradas) -> OperationResult:
        ...
        if not linhas_filtradas:
            return OperationResult.fail(
                "Nenhuma linha fornecida para geração de payloads."
            )
        ...
        for linha in linhas_filtradas:
            ...
            try:
                ...
                if not res_matricula or not res_matricula.success:
                    logger.info(
                        f"⚠️ Pulando {matricula_input}: {res_matricula.message if res_matricula else 'Erro na busca de matrícula'}"
                    )
                    erros += 1
                    continue
                ...
                id_profissional = buscar_id_seguro(
                    "MEDICOS",
                    linha.get("CRM Médico assistente"),
                    "numeroConselho",
                    linha.get("Médico assistente"),
                    "nome",
                )
                id_cid = buscar_id_seguro("CID", linha.get("CID"), "codigo")
                id_tipo_atestado = buscar_id_seguro(
                    "TIPOS_ATESTADO", linha.get("Tipo de atestado"), "descricao"
                )
                id_motivo = buscar_id_seguro(
                    "MOTIVO_CONSULTA", linha.get("Motivo da Consulta"), "descricao"
                )
                id_tipo_afastamento = buscar_id_seguro(
        ...
        if not lista_payloads:
            return OperationResult.fail(
                f"Falha ao gerar payloads. {erros} erros encontrados."
            )
        ...
        for nome_arquivo in lista_arquivos:
            ...
            if os.path.isfile(caminho_completo) and not nome_arquivo.startswith("~$"):
                try:
                    ...
                    dados_retorno = (
                        res_upload.data if hasattr(res_upload, "data") else res_upload
                    )
                    if dados_retorno:
        ...
        return [
            {
                ...
                "tipoDocumento": {"id": 1781, "descricao": "ATESTADO", "version": 0},
    ...
    def upload_arquivo_betha(self, caminho_arquivo) -> OperationResult:
        ...
        try:
            ...
            tipo_mime = (
                mimetypes.guess_type(caminho_arquivo)[0] or "application/octet-stream"
            )
            tamanho_mb = os.path.getsize(caminho_arquivo) / (1024 * 1024)
            if tamanho_mb > 10:
                return OperationResult.fail(
                    f"📁 Arquivo muito grande ({tamanho_mb:.2f}MB). Limite sugerido: 10MB."
                )

            with open(caminho_arquivo, "rb") as f:
                files = {"file": (nome_arquivo, f, tipo_mime)}
                ...
                if response.status_code in [200, 201]:
                    dados = response.json()
                    return OperationResult.ok(
                        f"✅ Upload concluído: {nome_arquivo}", data=dados
                    )

                if response.status_code in [401, 403]:
                    return OperationResult.fail(
                        "🔑 Token expirado ao tentar fazer upload. Atualize o acesso."
                    )

                return OperationResult.fail(
                    f"❌ Erro Betha ({response.status_code}): {response.text}"
                )

        except FileNotFoundError:
            return OperationResult.fail(
                f"🔍 Arquivo não encontrado para upload: {caminho_arquivo}"
            )
        except requests.exceptions.Timeout:
            return OperationResult.fail(
                f"⏳ Tempo esgotado ao enviar '{nome_arquivo}'. Internet muito lenta ou arquivo pesado."
            )
        except Exception as e:
            return ErrorTranslator.traduzir(e)
    ...
    def enviar_atestado(self, payload) -> OperationResult:
        """
        Envia o atestado para a Betha.
        Retorna success=True apenas para HTTP 200.
        O campo 'message' conterá sempre o JSON completo da resposta.
        """
        if not self.inicializado:
            return OperationResult.fail(
                json.dumps({"erro": self.erro_inicializacao}, ensure_ascii=False)
            )

        try:
            base_url = get_config("betha", "api", "base_url").rstrip("/")
            endpoint = get_config("betha", "api", "endpoints", "atestado").lstrip("/")
            url = f"{base_url}/{endpoint}"
        except Exception as e:
            return OperationResult.fail(
                json.dumps(
                    {"erro_config": f"Falha ao carregar URL: {str(e)}"},
                    ensure_ascii=False,
                )
            )
        ...
        try:
            response = requests.post(
                url, headers=self.headers, json=payload, timeout=45, proxies=PROXIES_OFF
            )
            ...
            json_string_completa = json.dumps(res_json, ensure_ascii=False)

            if response.status_code == 200:
                return OperationResult.ok(json_string_completa, data=res_json)

            return OperationResult.fail(json_string_completa)

        except requests.exceptions.Timeout:
            return OperationResult.fail(
                json.dumps(
                    {"erro": "Tempo de resposta esgotado (Timeout)"}, ensure_ascii=False
                )
            )
        except requests.exceptions.ConnectionError:
            return OperationResult.fail(
                json.dumps(
                    {"erro": "Falha de conexão com o servidor Betha"},
                    ensure_ascii=False,
                )
            )
        except Exception as e:
            return OperationResult.fail(
                json.dumps({"erro_inesperado": str(e)}, ensure_ascii=False)
            )
